...
async function loadFilterOptions(){
  ...
  try{
    ...
    renderMajorOptions(majorOptionsCache);
  }catch(e){
...
function renderMajorOptions(majors){
  ...
  const current = selectedMultiValues('allowedMajors');

  select.innerHTML = majors.map(m => `<option value="${m}">${m}</option>`).join('');
...
function filterSelectOptions(selectId, keyword){
  ...
  if(selectId === 'allowedMajors'){
    const filtered = !q
      ? majorOptionsCache
      : majorOptionsCache.filter(m => String(m).toLowerCase().includes(q));
    renderMajorOptions(filtered);
  }
}

function toggleAllSelect(checkboxId, selectId){
...
function selectedSelectOrAll(checkboxId, selectId){
  const checkbox = document.getElementById(checkboxId);

  if(checkbox && checkbox.checked) return 'ทั้งหมด';

  const values = selectedMultiValues(selectId);

  if(!values.length){
    throw new Error('กรุณาเลือกอย่างน้อย 1 รายการ หรือเลือก “ทั้งหมด”');
  }

  return values.join(',');
}

function setSelectValues(checkboxId, selectId, csv){
  const checkbox = document.getElementById(checkboxId);
  const select = document.getElementById(selectId);
  if(!checkbox || !select) return;
...
function toggleStudentSpecific(){
  const all = document.getElementById('allStudents').checked;
  const box = document.getElementById('studentSpecificBox');
...
async function searchParticipantStudents(){
  ...
  participantSearchTimer = setTimeout(async () => {
    ...
    try{
      const r = await api('searchStudents', {...adminPayload(), keyword});
      ...
      box.innerHTML = students.map(s => `
        <div class="search-result-item">
          ...
          <button type="button" onclick="addParticipantStudentId('${s.studentId}', '${String(s.fullName || '').replace(/'/g,'')}')">เลือก</button>
        </div>
      `).join('');
...
function addParticipantStudentId(id, name=''){
  ...
  document.getElementById('studentSpecificBox').classList.remove('hidden');
  renderParticipantStudentIds();
}
...
function renderParticipantStudentIds(){
  ...
  box.innerHTML = selectedParticipantIds.map(s => `
    <span class="tag">${s.id}${s.name ? ' ' + s.name : ''}<button type="button" onclick="removeParticipantStudentId('${s.id}')">×</button></span>
  `).join('');
}
...
function selectedParticipantStudentIds(){
  ...
  if(all) return 'ทั้งหมด';

  if(!selectedParticipantIds.length){
    throw new Error('กรุณาเลือกนักศึกษาอย่างน้อย 1 คน หรือเลือก “นักศึกษาทุกคน”');
  }
...
function setParticipantStudentIds(csv){
  const value = String(csv || 'ทั้งหมด').trim();
  const allCheckbox = document.getElementById('allStudents');
  const box = document.getElementById('studentSpecificBox');
  ...
  selectedParticipantIds = value.split(',').map(id => ({ id:id.trim(), name:'' })).filter(x => x.id);
...
async function saveEvent(){
  try{
    const data = {
      ...
      publishDate:val('publishDate'), level:val('level'), allowedYears:selectedYears()||'ทุกชั้นปี', allowedFaculties:selectedSelectOrAll('allFaculties','allowedFaculties'), allowedMajors:selectedSelectOrAll('allMajors','allowedMajors'), allowedStudentIds:selectedParticipantStudentIds()
    };
...
function editEvent(e){
  ...
  setSelectValues('allFaculties','allowedFaculties', e.allowedFaculties || 'ทั้งหมด');
  setSelectValues('allMajors','allowedMajors', e.allowedMajors || 'ทั้งหมด');
...
function clearEventForm(){document.querySelectorAll('#adminEventsPage input,#adminEventsPage textarea').forEach(el=>el.value='');document.querySelectorAll('.year-btn').forEach(b=>b.classList.remove('active'));document.querySelector('.year-btn[data-year="ทุกชั้นปี"]').classList.add('active');
  setSelectValues('allFaculties','allowedFaculties','ทั้งหมด');
  setSelectValues('allMajors','allowedMajors','ทั้งหมด');
