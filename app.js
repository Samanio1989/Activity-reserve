...
let currentEligibilityMode = 'year';
...
function selectedEligibilityMode() {
  const checked = document.querySelector('input[name="eligibilityMode"]:checked');
  return checked ? checked.value : currentEligibilityMode || 'year';
}

function setEligibilityMode(mode) {
  currentEligibilityMode = mode || 'year';

  ['eligibilityYearBox', 'eligibilityFacultyBox', 'eligibilityMajorBox', 'eligibilityStudentBox']
    .forEach(id => {
      const el = document.getElementById(id);
  ...
  const target = document.getElementById(
    mode === 'year' ? 'eligibilityYearBox' :
    mode === 'faculty' ? 'eligibilityFacultyBox' :
    mode === 'major' ? 'eligibilityMajorBox' :
    'eligibilityStudentBox'
  );
...
function selectedParticipantStudentIdsRequired() {
  if (!selectedParticipantIds.length) {
    throw new Error('กรุณาเพิ่มรหัสนักศึกษาอย่างน้อย 1 คน');
  }
  return selectedParticipantIds.map(s => s.id).join(',');
}
...
function updatePermissionPreview() {
  ...
  const mode = selectedEligibilityMode();

  previewYears.innerHTML = mode === 'year'
    ? `<div class="preview-item"><b>เงื่อนไขที่เลือก :</b> ตามชั้นปี<br><b>ชั้นปี :</b> ${selectedYears() || 'ทุกชั้นปี'}</div>`
    : '';

  previewFaculties.innerHTML = mode === 'faculty'
    ? `<div class="preview-item"><b>เงื่อนไขที่เลือก :</b> ตามคณะ<br><b>คณะ :</b> ${requiredMultiSelectDisplay('allowedFaculties')}</div>`
    : '';

  previewMajors.innerHTML = mode === 'major'
    ? `<div class="preview-item"><b>เงื่อนไขที่เลือก :</b> ตามสาขา<br><b>สาขา :</b> ${requiredMultiSelectDisplay('allowedMajors')}</div>`
    : '';

  previewStudents.innerHTML = mode === 'student'
    ? `<div class="preview-item"><b>เงื่อนไขที่เลือก :</b> เฉพาะรายรหัสนักศึกษา<br><b>นักศึกษาเฉพาะราย :</b><br>${participantStudentDisplay()}</div>`
    : '';
}
...
async function saveEvent() {
  try {
    const data = {
      ...
      level: val('level'),
      eligibilityMode: selectedEligibilityMode(),
      allowedYears: selectedEligibilityMode() === 'year' ? (selectedYears() || 'ทุกชั้นปี') : 'ทุกชั้นปี',
      allowedFaculties: selectedEligibilityMode() === 'faculty' ? requiredMultiSelect('allowedFaculties', 'คณะ') : 'ทั้งหมด',
      allowedMajors: selectedEligibilityMode() === 'major' ? requiredMultiSelect('allowedMajors', 'สาขา') : 'ทั้งหมด',
      allowedStudentIds: selectedEligibilityMode() === 'student' ? selectedParticipantStudentIdsRequired() : 'ทั้งหมด'
    };
...
function eligibilityModeLabel(mode) {
  if (mode === 'faculty') return 'ตามคณะ';
  if (mode === 'major') return 'ตามสาขา';
  if (mode === 'student') return 'เฉพาะรายรหัสนักศึกษา';
  return 'ตามชั้นปี';
}
        ...
        <tbody>
          ${allEvents.map(e => `
            <tr>
              ...
              <td>
                เงื่อนไข: ${eligibilityModeLabel(e.eligibilityMode || 'year')}<br>
...
function editEvent(e) {
  ...
  currentEligibilityMode = e.eligibilityMode || 'year';
  const modeRadio = document.querySelector(`input[name="eligibilityMode"][value="${currentEligibilityMode}"]`);
  if (modeRadio) modeRadio.checked = true;
  setEligibilityMode(currentEligibilityMode);
...
function clearEventForm() {
  ...
  currentEligibilityMode = 'year';
  const yearModeRadio = document.querySelector('input[name="eligibilityMode"][value="year"]');
  if (yearModeRadio) yearModeRadio.checked = true;
  setEligibilityMode('year');
