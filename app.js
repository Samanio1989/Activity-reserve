...
function updateParticipationHint() {
  ...
  if (capacityBox) capacityBox.classList.toggle('hidden-soft', privateMode);
  if (capacity) {
    capacity.required = !privateMode;
    if (privateMode) capacity.value = '';
  }

  if (!box) return;

  if (privateMode) {
    box.innerHTML = 'เฉพาะกลุ่ม: ไม่ต้องกำหนดจำนวนรับ ไม่ต้องจอง ผู้ที่ผ่านเงื่อนไขชั้นปี / คณะ / สาขา / รายบุคคล สามารถ Check-in / Check-out ได้ทันที';
    box.classList.add('private');
  } else {
    box.innerHTML = 'Open: เปิดให้จองตามจำนวนผู้เข้าร่วมที่กำหนด ใช้สิทธิ์ชั้นปี / คณะ / สาขา และต้องจองก่อนจึงจะ Check-in / Check-out ได้';
