...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loginSection) {
    loginSection.addEventListener('click', (event) => {
      if (event.target === loginSection) closeLoginPopup();
    });
  }

  if (typeof updateParticipationHint === 'function') updateParticipationHint();
});

function checkIn(eventId) { return doCheckIn(eventId); }
function checkOut(eventId) { return doCheckOut(eventId); }

function isPrivateEvent(e) { return String(e.participationType || e.eventType || 'open').toLowerCase() === 'private' || String(e.participationType || '').toLowerCase() === 'group'; }
function isOpenEvent(e) { return !isPrivateEvent(e); }
function eventAlreadyBooked(e) { return !!(e.isBooked || e.bookingId || ['booked','checked-in','completed','eligible'].includes(String(e.bookingStatus || e.status || '').toLowerCase())); }
...
function evidencePhotoHtml(url, label) {
  if (!url) return '';
  const imgUrl = (typeof drivePreviewUrl === 'function') ? drivePreviewUrl(url) : url;
  return `<div class="evidence-photo-wrap"><a href="${url}" target="_blank"><img src="${imgUrl}" class="evidence-photo" alt="${label}"></a><div class="small">${label}</div></div>`;
}

function selectReportEvent(eventId) {
  currentReportEventId = eventId;
  document.querySelectorAll('.report-card, .admin-event-card').forEach(card => card.classList.remove('selected'));
  document.querySelectorAll(`[data-report-event-id="${eventId}"]`).forEach(card => card.classList.add('selected'));
