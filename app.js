...
function buildMenu() {
  ...
  if (currentRole === 'admin') {
    menu.innerHTML = `
      <button class="tab active" onclick="showPage('adminCalendarPage')">📅 ปฏิทินกิจกรรม</button>
      <button class="tab" onclick="showPage('adminEventsPage')">🛠️ จัดการกิจกรรม</button>
...
async function loadAdminCalendar() {
  ...
  const monthEvents = events.filter(e => {
    const d = parseLocalDate(e.eventDate);
    return d && d.getFullYear() === year && d.getMonth() === month;
  });

  setText('adminTotalEvents', events.length);
  setText('adminTodayEventsCount', events.filter(e => e.eventDate === today && !isPastEvent(e)).length);
  setText('adminUpcomingEventsCount', events.filter(e => e.eventDate > today).length);
  setText('adminCompletedEventsCount', events.filter(e => isPastEvent(e)).length);

  renderAdminMonthCalendar(monthEvents);
  renderAdminMonthEventList(monthEvents);
}
...
function goCurrentAdminMonth() {
  adminCalendarDate = new Date();
  loadAdminCalendar();
}

function renderAdminMonthCalendar(monthEvents) {
  const title = document.getElementById('adminMonthTitle');
  const grid = document.getElementById('adminMonthCalendar');
  if (!title || !grid) return;

  const y = adminCalendarDate.getFullYear();
  const m = adminCalendarDate.getMonth();
  const thaiMonths = ['มกราคม','กุมภาพันธ์','มีนาคม','เมษายน','พฤษภาคม','มิถุนายน','กรกฎาคม','สิงหาคม','กันยายน','ตุลาคม','พฤศจิกายน','ธันวาคม'];
  title.innerText = `ปฏิทินกิจกรรม ${thaiMonths[m]} ${y + 543}`;
  ...
  const cells = [];
  for (let i = 0; i < startBlank; i++) cells.push('<div class="month-day blank"></div>');

  for (let day = 1; day <= totalDays; day++) {
    const dateStr = `${y}-${String(m + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    const events = monthEvents.filter(e => e.eventDate === dateStr).sort(sortEventsAsc);

    cells.push(`
      <div class="month-day ${dateStr === todayISO() ? 'is-today' : ''}">
        <div class="month-day-number">${day}</div>
        <div class="month-day-events">
          ${events.map(e => `
            <button class="month-event-pill ${isPastEvent(e) ? 'past' : 'upcoming'}" onclick="openEventDetailModalById('${e.eventId}')">
              <span>${e.startTime || '--:--'}</span> ${e.title || '-'}
            </button>
          `).join('')}
        </div>
      </div>
...
function renderAdminMonthEventList(monthEvents) {
  const box = document.getElementById('adminMonthEventList');
  if (!box) return;

  box.innerHTML = monthEvents.length
    ? monthEvents.sort(sortEventsAsc).map(e => adminManageEventCard(e)).join('')
    : '<div class="empty-state">เดือนนี้ยังไม่มีกิจกรรมในระบบ</div>';
}
...
async function saveEvent() {
  try {
    const privateMode = val('participationType') === 'private';

    
    const participationType = val('participationType') || 'open';
    if (participationType === 'private') {
      const hasEligibility = selectedYears() || selectedMultiValues('allowedFaculties').length || selectedMultiValues('allowedMajors').length || selectedParticipantIds.length;
      if (!hasEligibility) throw new Error('กิจกรรมเฉพาะกลุ่มต้องเลือกสิทธิ์อย่างน้อย 1 อย่าง: ชั้นปี คณะ สาขา หรือรายบุคคล');
    }
    ...
    const data = {
      ...
      endTime: val('endTime'),
      capacity: privateMode ? '' : val('capacity'),
      ...
      eventType: privateMode ? 'GROUP' : 'OPEN',
      location: val('location'),
      checkinMapLink: val('checkinMapLink'),
      checkoutMapLink: val('checkoutMapLink'),
      checkinRadiusMeters: val('checkinRadiusMeters') || '200',
      checkoutRadiusMeters: val('checkoutRadiusMeters') || '200',
      ...
      allowedStudentIds: privateMode ? selectedParticipantStudentIds() : 'ทั้งหมด'
    };
  ...
  return `
    <div class="admin-event-card" onclick="openEventDetailModalById('${e.eventId}')">
      ...
      <div class="admin-event-card-body">
        <div>📍 ${e.location || '-'}</div>
        <div>👥 ${String(e.participationType || 'open') === 'private' ? 'ผู้มีสิทธิ์ ' + (e.eligibleCount || 0) + ' คน' : 'จอง ' + (e.booked || 0) + '/' + (e.capacity || 0)}</div>
        <div>📸 IN ${e.checkedIn || 0} / OUT ${e.completed || 0}</div>
        <div>📍 ระยะ IN ${e.checkinRadiusMeters || 200} ม. / OUT ${e.checkoutRadiusMeters || 200} ม.</div>
...
function openEventDetailModal(e) {
  ...
  document.getElementById('eventDetailBody').innerHTML = `
    <div class="detail-grid">
      ...
      <div><b>จำนวนรับ/ผู้มีสิทธิ์</b><br>${String(e.participationType || 'open') === 'private' ? 'ผู้มีสิทธิ์ ' + (e.eligibleCount || 0) + ' คน' : (e.capacity || '-') + ' คน'}</div>
      ...
      <div><b>ระยะ Check-in</b><br>${e.checkinRadiusMeters || 200} เมตร</div>
      <div><b>Check-out</b><br>${cleanTimeDisplay(e.checkoutStart)} - ${cleanTimeDisplay(e.checkoutEnd)} น.</div>
      <div><b>ระยะ Check-out</b><br>${e.checkoutRadiusMeters || 200} เมตร</div>
...
function clearEventForm() {
  ...
  if (document.getElementById('checkinRadiusMeters')) document.getElementById('checkinRadiusMeters').value = '200';
  if (document.getElementById('checkoutRadiusMeters')) document.getElementById('checkoutRadiusMeters').value = '200';
  updateParticipationHint();
...
async function loadStudentEvents() {
  ...
  document.getElementById('todayEvents').innerHTML = todayEvents.length
    ...
    : '<div class="empty-state">วันนี้ยังไม่มีกิจกรรมที่เปิดให้เข้าร่วม</div>';
...
function eventCard(e, groupClass = 'upcoming') {
  const privateMode = String(e.participationType || 'open') === 'private';
  const typeLabel = privateMode ? 'เฉพาะกลุ่ม' : 'Open';
  const countText = privateMode
    ? `ผู้มีสิทธิ์ ${e.eligibleCount || 0} คน`
    : `จอง ${e.booked || 0}/${e.capacity || 0}`;
  ...
  return `
    <div class="event ${groupClass}">
      ...
      <div class="event-actions">
        ${privateMode
          ? `<button onclick="doCheckIn('${e.eventId}')">Check-in</button>
             <button class="btn-dark" onclick="doCheckOut('${e.eventId}')">Check-out</button>
             <span class="badge private-badge">ไม่ต้องจอง</span>`
...
async function doCheckIn(eventId) {
  try {
    ...
    const r = await submitFormApi('checkIn', {
      ...
      lng: loc.lng,
      photoBase64: photo
    await refreshStudentAfterCheckAction();
    });
...
function reportEventCard(e) {
  return `
    <div class="admin-event-card report-card"
         data-report-event-id="${e.eventId}"
         onclick="selectReportEvent('${e.eventId}')">
      <div class="admin-event-card-head">
        <div>
          <h3>${e.title || '-'}</h3>
          <p>${e.eventDate || '-'} | ${e.startTime || '-'} - ${e.endTime || '-'} น.</p>
      ...
      <div class="admin-event-card-actions" onclick="event.stopPropagation()">
        <button type="button" onclick="selectReportEvent('${e.eventId}')">ดูรายงาน</button>
      </div>
    </div>
  `;
}
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>ชื่อ</th>
              <th>สถานะ</th>
              <th>เวลา / Location Check-in</th>
              <th>รูป Check-in</th>
              <th>เวลา / Location Check-out</th>
              <th>รูป Check-out</th>
            </tr>
          </thead>
          <tbody>
...
function updateParticipationHint() {
  const type = val('participationType') || 'open';
  const hint = document.getElementById('participationHint');
  const capacityBox = document.getElementById('capacityBox');
  const eligibilityBox = document.getElementById('eligibilityBox');

  if (type === 'open') {
    if (hint) hint.innerHTML = 'Open: เปิดให้จองตามจำนวนรับ นักศึกษาที่จองแล้วเท่านั้นจึงจะ Check-in / Check-out ได้';
    if (capacityBox) capacityBox.classList.remove('hidden');
    if (eligibilityBox) eligibilityBox.classList.add('muted-box');
  } else {
    if (hint) hint.innerHTML = 'เฉพาะกลุ่ม: ต้องกำหนดสิทธิ์อย่างน้อย 1 อย่าง และผู้มีสิทธิ์ไม่ต้องจอง สามารถ Check-in / Check-out ได้เลย';
    if (capacityBox) capacityBox.classList.add('hidden');
    if (eligibilityBox) eligibilityBox.classList.remove('muted-box');
  }
}


async function refreshStudentAfterCheckAction() {
  try {
    await loadStudentEvents();
    await loadMyBookings();
  } catch (err) {
    console.warn('refreshStudentAfterCheckAction', err.message);
  }
}
...
function selectReportEvent(eventId) {
  ...
  document.querySelectorAll('.report-card, .admin-event-card')
    .forEach(card => card.classList.remove('selected'));

  document.querySelectorAll(`[data-report-event-id="${eventId}"]`)
...
function isPrivateEvent(e) {
  return String(e.participationType || e.eventType || 'open').toLowerCase() === 'private'
      || String(e.participationType || '').toLowerCase() === 'group';
}

function isOpenEvent(e) {
  return !isPrivateEvent(e);
}

function eventAlreadyBooked(e) {
  return !!(e.isBooked || e.bookingId || ['booked','checked-in','completed','eligible'].includes(String(e.bookingStatus || e.status || '').toLowerCase()));
}

function shouldShowForStudent(e) {
  if (isPrivateEvent(e)) return !!e.isEligible || !!e.canCheckDirect || eventAlreadyBooked(e);
  return true;
}

function canShowBookButton(e) {
  return isOpenEvent(e) && !eventAlreadyBooked(e);
}

function evidencePhotoHtml(url, label) {
  if (!url) return '';
  return `<div class="evidence-photo-wrap"><a href="${url}" target="_blank"><img src="${drivePreviewUrl ? drivePreviewUrl(url) : url}" class="evidence-photo" alt="${label}"></a><div class="small">${label}</div></div>`;
}


async function loadStudentEvents() {
  const r = await api('getEvents', { studentId: currentStudent.studentId });
  const events = (r.events || []).filter(shouldShowForStudent).sort(sortEventsAsc);
  allEvents = events;

  const today = todayISO();

  const todayCheck = events.filter(e =>
    e.eventDate === today &&
    (eventAlreadyBooked(e) || (isPrivateEvent(e) && (e.isEligible || e.canCheckDirect))) &&
    !isPastEvent(e)
  );

  const todayOpen = events.filter(e =>
    e.eventDate === today &&
    isOpenEvent(e) &&
    !eventAlreadyBooked(e) &&
    !isPastEvent(e)
  );
  ...
  const checkBox = document.getElementById('todayCheckActivities');
  if (checkBox) {
    checkBox.innerHTML = todayCheck.length
      ? todayCheck.map(e => studentCheckCard(e)).join('')
      : '<div class="empty-state">วันนี้ยังไม่มีกิจกรรมที่ต้อง Check-in / Check-out</div>';
  }

  const todayBox = document.getElementById('todayEvents');
  if (todayBox) {
    todayBox.innerHTML = todayOpen.length
      ? todayOpen.map(e => studentEventCard(e, 'today')).join('')
      : '<div class="empty-state">วันนี้ยังไม่มีกิจกรรมที่เปิดให้จอง</div>';
  }

  const upBox = document.getElementById('upcomingEvents');
  if (upBox) {
    upBox.innerHTML = upcoming.length
      ? upcoming.map(e => studentEventCard(e, 'upcoming')).join('')
      : '<div class="empty-state">ยังไม่มีกิจกรรมที่กำลังจะมาถึง</div>';
  }

  const pastBox = document.getElementById('pastEvents');
  if (pastBox) {
    pastBox.innerHTML = past.length
      ? past.map(e => studentEventCard(e, 'completed', false)).join('')
      : '<div class="empty-state">ยังไม่มีกิจกรรมที่ผ่านมา</div>';
  }
}

function studentEventCard(e, cls = 'upcoming', showAction = true) {
  return `
    <div class="event ${cls}">
      <h3>${e.title || '-'}</h3>
      <div class="small">${e.eventDate || '-'} | ${e.startTime || '-'} - ${e.endTime || '-'} น.</div>
      <div class="status">
        <span class="badge">${isPrivateEvent(e) ? 'เฉพาะกลุ่ม' : 'Open'}</span>
        <span class="badge">${e.level || '-'}</span>
        <span class="badge">${e.activityType || '-'}</span>
        ${isOpenEvent(e) ? `<span class="badge">จอง ${e.booked || 0}/${e.capacity || 0}</span>` : '<span class="badge">ไม่ต้องจอง</span>'}
      </div>
      <div class="time-box">
        Check-in: ${cleanTimeDisplay(e.checkinStart)} - ${cleanTimeDisplay(e.checkinEnd)} น.<br>
        Check-out: ${cleanTimeDisplay(e.checkoutStart)} - ${cleanTimeDisplay(e.checkoutEnd)} น.
      </div>
      ${showAction && canShowBookButton(e) ? `<button onclick="bookEvent('${e.eventId}')">จองกิจกรรม</button>` : ''}
      ${showAction && eventAlreadyBooked(e) ? `<button class="btn-light" disabled>จองแล้ว</button>` : ''}
    </div>
  `;
}

function studentCheckCard(e) {
  return `
    <div class="event today">
      ...
      <div class="status">
        สถานะ: <b>${displayBookingStatus(e.bookingStatus || e.status || (isPrivateEvent(e) ? 'eligible' : 'booked'))}</b><br>
        Check-in: ${e.checkinAt || '-'}<br>
        Check-out: ${e.checkoutAt || '-'}
        <div class="evidence-row">
          ${evidencePhotoHtml(e.checkinPhotoUrl, 'รูป Check-in')}
          ${evidencePhotoHtml(e.checkoutPhotoUrl, 'รูป Check-out')}
        </div>
      </div>
      <button onclick="checkIn('${e.eventId}')">Check-in</button>
      <button class="btn-dark" onclick="checkOut('${e.eventId}')">Check-out</button>
    </div>
  `;
}
